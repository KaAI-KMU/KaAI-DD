...
class UniformRandomSample(object):
    ...
    If the number of frames is less than 5 (about one second),
    ...
    def __call__(self, frame_indices):
        ...
        if len(frame_indices) < 5* self.end_sec:
            out.extend(list(range(len(frame_indices)-self.size,len(frame_indices))))
        else:
            sample_per_sec = self.size//self.end_sec
            sample_base = random.randint(0, 5-sample_per_sec-1)

            for i in range(0, self.end_sec-1):
                out.extend(list(range(sample_base+i*5, sample_base+i*5+sample_per_sec)))
            sample_rest = self.size-len(out)
            out.extend(list(range(sample_base+(self.end_sec-1)*5, sample_base+(self.end_sec-1)*5+sample_rest)))
        out_idx = []
        for idx in out:
...
class UniformEndSample(object):
    ...
    def __call__(self, frame_indices):
        ...
        if len(frame_indices) < 5*self.end_sec:
            ...
            sample_per_sec = self.size//self.end_sec
            sample_base = 5-sample_per_sec-1
            for i in range(0, self.end_sec-1):
                out.extend(list(range(sample_base+i*5, sample_base+i*5+sample_per_sec)))
            sample_rest = self.size-len(out)
            out.extend(list(range(sample_base+(self.end_sec-1)*5, sample_base+(self.end_sec-1)*5+sample_rest)))
