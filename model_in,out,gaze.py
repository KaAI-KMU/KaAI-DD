...
if __name__ == '__main__':
    ...
    #Gaze 모델 불러오기
    gaze_predictor = GazePredictorLSTM(input_dim=6, hidden_dim=256, num_layers=2, num_classes=5, drop_rate=0.5).to(device)
    gaze_predictor = nn.DataParallel(gaze_predictor, device_ids=None)

    criterion_gaze = nn.CrossEntropyLoss()
    optimizer_gaze = torch.optim.SGD(gaze_predictor.parameters(),lr=opt.learning_rate,
		momentum=opt.momentum,
		dampening=dampening,
		weight_decay=opt.weight_decay,
		nesterov=opt.nesterov_outside)
    scheduler_gaze = lr_scheduler.MultiStepLR(
        optimizer_gaze, milestones=opt.lr_step, gamma=0.1)
